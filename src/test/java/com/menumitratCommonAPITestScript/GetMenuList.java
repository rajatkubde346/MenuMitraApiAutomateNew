    ...
    public Object[][] getMenuListData() throws customException {
        try {
            ...
            if (filteredData.isEmpty()) {
                String errorMsg = "No valid positive menu list test data found after filtering";
                LogUtils.failure(logger, errorMsg);
                ExtentReport.getTest().log(Status.FAIL, MarkupHelper.createLabel(errorMsg, ExtentColor.RED));
                throw new customException(errorMsg);
            }
            ...
            LogUtils.info("Successfully retrieved " + obj.length + " positive menu list test scenarios");
            ExtentReport.getTest().log(Status.PASS, "Successfully retrieved " + obj.length + " positive menu list test scenarios");
            return obj;
        } catch (Exception e) {
